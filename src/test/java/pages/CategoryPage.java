...
public class CategoryPage {
    ...
    public void filterByParentAndWait(String parentName) {
        ...
        if (!rows.isEmpty()) firstRow = rows.get(0);
    ...
    public void sortBy(String column, String order) {
        ...
        for (int i = 0; i < clicks; i++) clickColumnHeaderAndWait(column);
    }
    ...
    private void sortByIdLinkAndWait(String order) {
        ...
        if (url != null && url.contains("sortField=id") && url.contains("sortDir=" + desiredDir)) return;
        ...
        WebElement link = wait.until(org.openqa.selenium.support.ui.ExpectedConditions.elementToBeClickable(desiredLink));
    ...
    private void sortByNameLinkAndWait(String order) {
        ...
        String url = driver.getCurrentUrl();
        if (url != null && url.contains("sortField=name") && url.contains("sortDir=" + desiredDir)) return;
        ...
        By linkBy = By.xpath("//table//th//a[normalize-space()='Name' and contains(@href,'sortField=name') and contains(@href,'sortDir=" + desiredDir + "')]");
        if (driver.findElements(linkBy).isEmpty()) {
            // Fallback if header text differs but href is correct
            linkBy = By.xpath("//table//a[contains(@href,'sortField=name') and contains(@href,'sortDir=" + desiredDir + "')]");
        }
    ...
    private void clickColumnHeaderAndWait(String column) {
        ...
        // Sorting is server-side (href), so URL often changes; otherwise table rows refresh
        try {
            wait.until(d -> {
